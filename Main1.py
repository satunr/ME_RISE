...
def main_score(G, G_E, Ac0, S, aP, K, Select):
    ...
    # AcList0 = [sparse.linalg.matrix_power(Ac0, k + 1) for k in range(K)]
...
for iterate in range(I):
    ...
    while True:
        ...
        # G = gen_weighted_random(n, pE, aP, 'Wiki-Vote.gml')

        G = nx.read_gml('Wiki_Now.gml')
        G = nx.convert_node_labels_to_integers(G, first_label=0)
    ...
    print (iterate, (time.time() - t0) / (iterate + 1))
